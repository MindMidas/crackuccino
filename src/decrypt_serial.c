...
void permute(
    ...
    for (int i = l; i <= r; i++) {
        ...
        permute(permutation, l + 1, r, ciphertext, input_dict, hd, decoded, total_leaves, valid_hits, rank, hits); // recurse
...
static void print_serial_usage(
    const char *prog
) {
...
int main(
    ...
    permute(permutation, 0, len - 1, ciphertext, input_dict, hd, decoded, &total_leaves, &valid_hits, 0, hits);
